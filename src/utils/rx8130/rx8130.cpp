...
void RX8130_Class::setAlarmTime(struct tm *time)
{
    uint8_t rbuf = 0;
    ...
    uint8_t date[3] = {dec2bcd(time->tm_min),  dec2bcd(time->tm_hour), dec2bcd(time->tm_mday)};

    writeRegister(RX8130_REG_ALMIN, date, 3);
...
void RX8130_Class::getAlarmTime(struct tm *time)
{
    uint8_t date[3];
    readRegister(RX8130_REG_ALMIN, date, 3);

    time->tm_min  = bcd2dec(date[0] & 0xff);
    time->tm_hour  = bcd2dec(date[1] & 0xff);
    time->tm_mday = bcd2dec(date[2] & 0xff);
}

uint8_t RX8130_Class::readIrqFlags()
{
    return readRegister8(RX8130_REG_FLAG);
}

void RX8130_Class::clearIrqFlags()
{
    writeRegister8(RX8130_REG_FLAG, 0b00000000);
}

void RX8130_Class::enableIrq(uint8_t bitmap)
{
    writeRegister8(RX8130_REG_CTRL0, bitmap);
}

void RX8130_Class::disableIrq()
{
    writeRegister8(RX8130_REG_CTRL0, 0b00000000);
}

uint8_t RX8130_Class::readReg(uint8_t reg)
{
